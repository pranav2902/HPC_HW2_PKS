...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  ...
  double C=0.0;
  #pragma omp parallel for shared(c) reduction(+:C)
    for (long p = 0; p < n; p+=BLOCK_SIZE) {
      for (long j = 0; j < k; j+=BLOCK_SIZE) {
        for (long i = 0; i < m; i+=BLOCK_SIZE) {
            for (long l = p; l < p+BLOCK_SIZE; l++) {
              for (long r = j; r < j+BLOCK_SIZE; r++) {
                for (long s = i; s < i+BLOCK_SIZE; s++) {
                  double A = a[l + n*s];
                  double B = b[s + k*r];
                  double C = c[l + m*r];
                  C = C + A*B;
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    flops = 2*m*k*n*NREPEATS*pow(10,-9)/time;
    double bandwidth = 0; // TODO: calculate from m, n, k, NREPEATS, time
    bandwidth = (k*n*n + 2*m*n+ m*k)*NREPEATS*pow(10,-9)/time;
